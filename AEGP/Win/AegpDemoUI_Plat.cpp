...
#include "AegpDemoUI_Plat.h"
#include <stdio.h>

const char OSWndObjectProperty[] = "AegpDemoUI_PlatPtr";

constexpr std::uint64_t BtnID {444};

AegpDemoUI_Plat::AegpDemoUI_Plat(SPBasicSuite* spbP, AEGP_PanelH panelH, 
									AEGP_PlatformViewRef platformWindowRef,
									AEGP_PanelFunctions1* outFunctionTable)
							: AegpDemoUI(spbP, panelH, platformWindowRef, outFunctionTable),
							i_prevWindowProc(NULL)
{
	...
	SetWindowLongPtrA(platformWindowRef, GWLP_WNDPROC, (LONG_PTR)AegpDemoUI_Plat::StaticOSWindowWndProc);
...
LRESULT CALLBACK	AegpDemoUI_Plat::StaticOSWindowWndProc(	HWND	hWnd, 
														  UINT	message, 
														  WPARAM	wParam, 
														  LPARAM	lParam)
{
	// suck out our window ptr
	AegpDemoUI_Plat* platPtr = reinterpret_cast<AegpDemoUI_Plat*>(::GetProp(hWnd, OSWndObjectProperty));
	if(platPtr){
...
LRESULT AegpDemoUI_Plat::OSWindowWndProc(	HWND	hWnd, 
															  UINT	message, 
															  WPARAM	wParam, 
															  LPARAM	lParam)
{
...
void AegpDemoUI_Plat::InvalidateAll()
{
	RECT clientArea;
