...
public:
	...
	{
		PT_XTE_START {
			reinterpret_cast<AegpDemo*>(refconP)->CommandHook(command, hook_priority, already_handledB, handledPB);
	...
	{
		PT_XTE_START {
			reinterpret_cast<AegpDemo*>(plugin_refconP)->UpdateMenuHook(active_window);
		} PT_XTE_CATCH_RETURN_ERR;
	}
	...
	{
		PT_XTE_START {
			 reinterpret_cast<AegpDemo*>(plugin_refconP)->CreatePanelHook(container, panelH, outFunctionTable, outRefcon);
		} PT_XTE_CATCH_RETURN_ERR;
	}
	...
	{
		PT_XTE_START {
			reinterpret_cast<AegpDemo*>(plugin_refconP)->DeathHook();
		} PT_XTE_CATCH_RETURN_ERR;
	}
			...
			i_match_nameZ((A_u_char*)STR(StrID_Name))
	{
		...
		PT_ETX(i_sp.RegisterSuite5()->AEGP_RegisterCommandHook(i_pluginID,
																AEGP_HP_BeforeAE,
																i_command,
																&AegpDemo::S_CommandHook,
																(AEGP_CommandRefcon)(this)));
		PT_ETX(i_sp.RegisterSuite5()->AEGP_RegisterUpdateMenuHook(i_pluginID,
																&AegpDemo::S_UpdateMenuHook,
																NULL));
		PT_ETX(i_sp.RegisterSuite5()->AEGP_RegisterDeathHook(i_pluginID,
																&AegpDemo::S_DeathHook,
																NULL));

		PT_ETX(i_ps->AEGP_RegisterCreatePanelHook(i_pluginID, i_match_nameZ,
													&AegpDemo::S_CreatePanelHook,
	...
	void CreatePanelHook(
		...
		AEGP_PanelRefcon*		outRefcon)
	{
		*outRefcon = reinterpret_cast<AEGP_PanelRefcon>(new AegpDemoUI_Plat(i_pica_basicP, panelH, container, outFunctionTable));
	}
...
{
	PT_XTE_START{
		*global_refconP = (AEGP_GlobalRefcon) new AegpDemo(pica_basicP, aegp_plugin_id);
	} PT_XTE_CATCH_RETURN_ERR;
}
